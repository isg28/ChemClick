...
function LessonOnePointFour(){
    ...
    const generateRandomNumberAndPosition = () => {
        // Generate random number between 0.0 and 5.99
    ...
    const handleMouseMove = (e) => {
        if (isDragging) {
            ...
            if (newLeft >= 85 && newLeft <= 889) { // Adjust boundaries as per container width
                document.querySelector('.lesson-one-point-four-starting-measurement').style.left = `${newLeft}px`;
    ...
    const handleSubmit = () => {
        const targetPosition = 85 + (randomNumber / 5.99) * 790;
    ...
    return (
        <div className='lesson-one-point-four' onMouseMove={handleMouseMove}
            ...
            <div className='lesson-one-point-four-box'>
                ...
                <div className='lesson-one-point-four-content'>
                    ...
                    <div className='lesson-one-point-four-measurement-container'>
                        <div className = 'lesson-one-point-four-line' style = {{width: `${lineWidth}px`}}></div>
                        <img src ={require('../../assets/question/startingmeasurement.png')} className= "lesson-one-point-four-starting-measurement" alt= "Starting Measurment"
                ...
                <div className="submit-feedback-container">
                    <button className='lesson-one-point-four-submit' onClick={handleSubmit}>Submit Answer</button>
                    <div className={`lesson-one-point-four-feedback ${feedbackClass}`}>
                    <p>{feedbackMessage}</p>
                    </div>
                </div>
            </div>
